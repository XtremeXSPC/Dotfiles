# =========================================================================== #
# ---- CMake Toolchain File for Clang - Sanitizer Builds on macOS/Linux ----- #
# =========================================================================== #
#
# Description:
...
if(DEFINED CACHE{CACHED_CLANG_EXECUTABLE} AND EXISTS "${CACHED_CLANG_EXECUTABLE}")
    ...
    if(APPLE)
        # On macOS, prefer system clang (has best sanitizer support)
        set(COMPILER_SEARCH_NAMES 
            clang++     # System clang (preferred)
        ...
        # Linux
        set(COMPILER_SEARCH_NAMES 
            clang++-18 clang++-17 clang++-16 clang++-15 clang++-14 clang++-13 clang++
        )
        set(COMPILER_SEARCH_PATHS 
            /usr/bin
            /usr/local/bin
...
string(REPLACE "++" "" C_COMPILER_NAME2 ${CLANG_NAME})

find_program(C_COMPILER_PATH
    NAMES ${C_COMPILER_NAME} ${C_COMPILER_NAME2}
    HINTS ${CLANG_DIR}
    NO_DEFAULT_PATH
)
...
if(NOT C_COMPILER_PATH)
    # Try broader search
    find_program(C_COMPILER_PATH
        NAMES clang-18 clang-17 clang-16 clang-15 clang-14 clang
        PATHS ${COMPILER_SEARCH_PATHS}
    )
endif()
...
if(NOT HAS_ASAN OR NOT HAS_UBSAN)
    message(WARNING 
        "This Clang installation may lack full sanitizer support.\n"
        "Some sanitizers might not work properly.")
endif()
