...
# @internal
# @description Delegates a Python sync subcommand to the extensions module
# with standard arguments (source, destination, --home).
# @arg $1 string Python subcommand name (e.g. sync-status, sync-setup).
# @arg $@ string Additional arguments forwarded to the Python script.
# @exitcode 1 The exit code from the delegated Python subcommand.
# -----------------------------------------------------------------------------
_vscode_sync_run_python_command() {
  local subcommand="$1"
  shift
...
# @internal
# @description Prints usage help for the vscode_sync_update command.
# @noargs
# -----------------------------------------------------------------------------
_vscode_sync_update_usage() {
  printf "%s\n" \
...
# vscode_sync_setup
# -----------------------------------------------------------------------------
# @description Mirrors Stable extensions into Insiders via symlinks.
# Requires VS Code to be closed and snapshots profile state before changes.
# @noargs
# @exitcode 1 If platform, Python, lock, or VS Code checks fail.
# -----------------------------------------------------------------------------
vscode_sync_setup() {
...
# vscode_sync_update
# -----------------------------------------------------------------------------
# @description Reconciles the shared extensions root and Insiders state.
# Dry-run previews changes without a lock; normal mode requires VS Code closed.
# @option -n | --dry-run Preview actions without changing anything.
# @option --skip-clean Skip duplicate cleanup in the shared Stable root.
# @option -h | --help Show this help.
# @exitcode 1 If platform, Python, lock, process, or extension checks fail.
# @exitcode 2 If an option is unknown.
# -----------------------------------------------------------------------------
vscode_sync_update() {
  setopt localoptions localtraps
  _shared_init_colors
...
# -----------------------------------------------------------------------------
# @description Alias for vscode_sync_update; forwards all options unchanged.
# @arg $@ string Options forwarded to vscode_sync_update.
# @exitcode 1 If the delegated update fails.
# -----------------------------------------------------------------------------
vscode_update_extensions() {
  vscode_sync_update "$@"
}
...
# vscode_sync_status
# -----------------------------------------------------------------------------
# @description Displays sync state through the Python sync-status command.
# No lock is acquired, so it is safe while VS Code is open.
# @noargs
# @exitcode 1 If platform, Python, or status checks fail.
# -----------------------------------------------------------------------------
vscode_sync_status() {
  _shared_init_colors
  _vscode_sync_check_platform || return 1
...
# vscode_sync_check
# -----------------------------------------------------------------------------
# @description Reports sync health from Python checks, processes, and backups.
# Returns healthy, degraded, or unhealthy status and prints the environment.
# @noargs
# @exitcode 1 If health is unhealthy or a prerequisite check fails.
# -----------------------------------------------------------------------------
vscode_sync_check() {
  _shared_init_colors
  _vscode_sync_check_platform || return 1
...
# vscode_sync_remove
# -----------------------------------------------------------------------------
# @description Interactively replaces Insiders symlinks with independent copies.
# Requires VS Code closed and snapshots profile state before changes.
# @noargs
# @exitcode 1 If platform, Python, lock, or VS Code checks fail.
# -----------------------------------------------------------------------------
vscode_sync_remove() {
...
# ============================================================================ #
# End of vscode/sync/commands.zsh
