...
class ApplyExtensionSetupTests(unittest.TestCase):
    ...
    def test_setup_restores_excluded_symlink_as_independent_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                (excluded_link / "marker.txt").read_text(encoding="utf-8"), "stable"
            )
    ...
    def test_setup_rolls_back_extension_tree_when_manifest_apply_aborts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                (unmanaged_dir / "marker.txt").read_text(encoding="utf-8"), "original"
            )
...
class ApplyExtensionRemoveTests(unittest.TestCase):
    ...
    def test_remove_restores_shared_entries_and_prunes_broken_symlinks(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                (shared_link / "marker.txt").read_text(encoding="utf-8"), "shared"
            )
            self.assertFalse((insiders_root / "ghost.ext-1.0.0").exists())
            self.assertTrue((insiders_root / "native.ext-1.0.0").is_dir())
            self.assertEqual(report.failed_paths, ())

    def test_remove_restores_exact_copy_without_nested_artifacts(self) -> None:
        """The restored directory must mirror the source exactly, with no staging leftovers."""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (shared_dir / "package.json").write_text(
                '{"name": "ext"}', encoding="utf-8"
            )
            (insiders_root / "foo.ext-1.0.0").symlink_to(shared_dir)

            report = apply_extension_remove(stable_root, insiders_root)

            self.assertEqual(report.restored_entry_copy_count, 1)
            restored = insiders_root / "foo.ext-1.0.0"
            self.assertFalse(restored.is_symlink())
            self.assertEqual(
                sorted(entry.name for entry in restored.iterdir()),
                ["package.json"],
            )
            self.assertEqual(
                sorted(entry.name for entry in insiders_root.iterdir()),
                ["foo.ext-1.0.0"],
            )

    def test_remove_skips_foreign_symlinks(self) -> None:
        """Symlinks that never pointed into the Stable root must be left untouched."""

        with tempfile.TemporaryDirectory() as temp_dir:
            home = Path(temp_dir)
            stable_root = home / ".vscode/extensions"
            insiders_root = home / ".vscode-insiders/extensions"
            external_dir = home / "dev-extension"
            stable_root.mkdir(parents=True)
            insiders_root.mkdir(parents=True)
            external_dir.mkdir()

            foreign_link = insiders_root / "custom.ext-1.0.0"
            foreign_link.symlink_to(external_dir)

            report = apply_extension_remove(stable_root, insiders_root)

            self.assertEqual(report.skipped_foreign_symlink_count, 1)
    ...
    def test_remove_restores_legacy_root_symlink_to_real_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                (insiders_root / "foo.ext-1.0.0" / "marker.txt").read_text(
                    encoding="utf-8"
                ),
                "shared",
            )
