...
def _unique_restore_staging_path(path: Path) -> Path:
    ...
    for attempt in range(1, _MAX_RESTORE_STAGING_ATTEMPTS + 1):
        candidate = path.parent / f".{path.name}.vscode_sync_restore.{os.getpid()}.{attempt}"
        if not candidate.exists() and not candidate.is_symlink():
            return candidate
...
class _ExtensionMutationTransaction:
    ...
    def capture_existing_path(self, path: Path, *, root: Path) -> bool:
        ...
        if not is_path_location_within_directory(path, root):
            raise ExtensionMutationError(f"path escaped managed root during setup: {path}")
        ...
        self._rollback_actions.append(
            lambda path=path, backup_path=backup_path, root=root: self._restore_backup_path(
                path,
                backup_path,
                root=root,
            )
    ...
    def record_move(
        ...
        *,
        ...
        self._rollback_actions.append(
            ...
            destination_root=destination_root: self._restore_moved_path(
                source_path,
                destination_path,
                source_root=source_root,
    ...
    def commit(self) -> None:
        """Best-effort cleanup of rollback backups after a successful apply."""
    ...
    def _restore_backup_path(path: Path, backup_path: Path, *, root: Path) -> None:
        ...
        if path.exists() or path.is_symlink():
            if not _safe_remove_path(path, root=root):
                raise RuntimeError(f"failed to clear path before rollback restore: {path}")
    ...
    def _remove_created_path(path: Path, *, root: Path) -> None:
        ...
        if path.exists() or path.is_symlink():
            if not _safe_remove_path(path, root=root):
                raise RuntimeError(f"failed to remove created path during rollback: {path}")
    ...
    def _restore_moved_path(
        ...
        *,
        ...
        if not is_path_location_within_directory(source_path, source_root):
            raise ValueError(f"source path escaped managed root during rollback: {source_path}")
        ...
        if source_path.exists() or source_path.is_symlink():
            if not _safe_remove_path(source_path, root=source_root):
                raise RuntimeError(f"failed to clear source path during rollback: {source_path}")
        if destination_path.exists() or destination_path.is_symlink():
            shutil.move(str(destination_path), str(source_path))


def _create_managed_symlink_or_raise(*, link_path: Path, target_path: Path, root: Path) -> None:
    """Create a managed symlink or raise when the operation cannot complete safely."""

    if not target_path.is_dir():
        raise ExtensionMutationError(f"expected Stable extension target is missing: {target_path}")
    try:
        created = _safe_create_symlink(link_path=link_path, target_path=target_path, root=root)
    except OSError as exc:
        raise ExtensionMutationError(f"failed to create managed symlink: {link_path}") from exc
    if not created:
        raise ExtensionMutationError(f"refusing to create symlink outside managed root: {link_path}")
...
def _move_managed_path_or_raise(
    *,
    ...
    if not is_path_location_within_directory(source_path, source_root):
        raise ExtensionMutationError(f"source path escaped managed root during setup: {source_path}")
    if not is_path_location_within_directory(destination_path, destination_root):
        raise ExtensionMutationError(
            f"destination path escaped managed root during setup: {destination_path}"
        )
    if not (source_path.exists() or source_path.is_symlink()):
        raise ExtensionMutationError(f"managed source path disappeared during setup: {source_path}")
...
def _restore_directory_copy_or_raise(
    *,
    ...
    if not is_path_location_within_directory(canonical_source, source_root):
        raise ExtensionMutationError(f"source path escaped managed root during restore: {source_path}")
    if not canonical_source.exists() or not canonical_source.is_dir():
        raise ExtensionMutationError(f"restore source is missing or not a directory: {source_path}")

    destination_path.parent.mkdir(parents=True, exist_ok=True)
    stage_path = _unique_restore_staging_path(destination_path)
    try:
        shutil.copytree(canonical_source, stage_path, symlinks=True)
        had_original = transaction.capture_existing_path(destination_path, root=destination_root)
    ...
    try:
        for decision in plan.decisions:
            ...
            if decision.action in {SymlinkAction.BROKEN, SymlinkAction.WRONG_TARGET}:
                had_original = transaction.capture_existing_path(path, root=insiders_root)
                _create_managed_symlink_or_raise(
                    link_path=path,
                    target_path=expected_target,
                    root=insiders_root,
